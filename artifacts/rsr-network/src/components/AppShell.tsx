...
export function AppShell({ children }: AppShellProps) {
  ...
  const standingColor = (standing?: string) => {
    ...
    if (standing === "Operator") return "text-sky-300";
    ...
    <div className="min-h-screen bg-[#050607] text-zinc-100 flex flex-col font-sans">
      ...
      <header className="h-14 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-sm flex items-center justify-between px-6 z-20 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="text-[10px] text-zinc-600 tracking-[0.2em] uppercase">Status: Secure</div>
        </div>

        <div className="flex items-center gap-4 text-xs text-zinc-500 font-mono uppercase tracking-wider">
          <div className="hidden sm:flex items-center gap-4">
            <div>Signals: <span className="text-zinc-300">{signals.length}</span></div>
            <div>Cases: <span className="text-zinc-300">{cases.length}</span></div>
          </div>
            ...
            <div className="relative" ref={menuRef}>
              ...
              >
                <div className="flex items-center gap-2">
                  <div className={cn("w-1.5 h-1.5 rounded-full shrink-0", isAdmin ? "bg-amber-400" : "bg-sky-400")} />
                ...
                <ChevronDown className={cn("w-3.5 h-3.5 text-zinc-600 transition-transform", menuOpen && "rotate-180")} />
              </button>

              {menuOpen && (
                <div className="absolute right-0 top-full mt-1 w-56 border border-zinc-800 bg-zinc-950 shadow-2xl z-50">
                  <div className="p-3 border-b border-zinc-800">
                    <div className={cn("text-xs font-medium uppercase tracking-wider", standingColor(currentUser.standing))}>
                      {currentUser.alias}
                    </div>
                    <div className="text-[10px] text-zinc-600 font-mono mt-0.5">{currentUser.id}</div>
                  ...
                  <div className="p-1">
                    <Link
                      ...
                      className="flex items-center gap-3 px-3 py-2 text-xs text-zinc-400 hover:bg-zinc-800/60 hover:text-zinc-100 transition-colors uppercase tracking-widest"
                    >
                      <UserCircle className="w-3.5 h-3.5 text-zinc-600" />
                      Operator Dossier
                    </Link>
                    <Link
                      href="/identity"
                  ...
                  <div className="p-1 border-t border-zinc-800">
                    <button
                      ...
                      className="flex items-center gap-3 px-3 py-2 text-xs text-red-500 hover:bg-red-950/30 hover:text-red-300 transition-colors uppercase tracking-widest w-full text-left"
                    >
      ...
      <div className="flex flex-1 overflow-hidden z-10 relative">
        {/* Left Rail */}
        <aside className="w-60 border-r border-zinc-800 bg-zinc-950/60 flex flex-col shrink-0">
          <nav className="flex-1 py-5 px-3 space-y-px overflow-y-auto rsr-scroll">
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.path} href={item.path} className={cn(
                  ...
                  active
                    ? "bg-sky-950/30 text-sky-400 border-l-2 border-sky-500/70"
                    : "text-zinc-500 hover:bg-zinc-900/40 hover:text-zinc-200 border-l-2 border-transparent"
                )}>
                  <item.icon className={cn("w-4 h-4 shrink-0", active ? "text-sky-400" : "text-zinc-600 group-hover:text-zinc-400")} />
        ...
        {/* Main Content */}
        <main className="flex-1 overflow-auto rsr-scroll bg-black/10">
